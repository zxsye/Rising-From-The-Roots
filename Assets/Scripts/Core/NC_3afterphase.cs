...
public class NC_3afterphase : MonoBehaviour
{
    List<string> data;
    ...
    void LoadChapterFile(string fileName) {
        ...
        var file = Resources.Load<TextAsset>("Story/" + fileName);
    ...
    void HandleLine(string line) {
        ...
        } else if (dialogueAndActions.Length == 5) {
            HandleDialogue(dialogueAndActions[0], dialogueAndActions[1]);
            HandleEventsFromLine(dialogueAndActions[2]);
            HandleEventsFromLine(dialogueAndActions[3]);
            HandleEventsFromLine(dialogueAndActions[4]);
        } else {
    ...
    void TurnOff(string characterName) {
        ...
        if (character == null) print("ERROR CANNOT TURN OFF NULL CHARACTER");
    ...
    void SetChar(string name) {
        Character c = CharacterManager.instance.GetCharacter(name);
        c.TurnOnOff(true);
        // c.SetSprite(type);
    }
